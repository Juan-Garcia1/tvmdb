...
const HorizontalTvShowCarousel = ({ carouselName, config, tvShows }) => {
  const settings = {
    dots: false,
    arrows: true,
    slidesToShow: 9,
      ...
      <Slider {...settings}>
        {tvShows.results.map(movie => (
          <figure key={movie.id} className="h-carousel-item">
            ...
            <figcaption>
              <h5 className="truncate truncate-small">{movie.name}</h5>
            </figcaption>
          </figure>
        ))}
      </Slider>
...
export default connect(mapStateToProps)(HorizontalTvShowCarousel);
